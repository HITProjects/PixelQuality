...
class PairedImageDataset_mk2(Dataset):
    """
    Dataset that returns pairs of images (clean, new) or (clean, ref) along with their score.
    Supports optional preloading of the entire dataset to a specified device (e.g., GPU).
    ...
    Args:
        root_dir (str): Directory with subfolders 'clean', 'new', 'ref'.
        metadata_json_path (str): Path to JSON file.
        transform (albumentations transform, optional): Default: dual ToTensorV2().
        preload (bool, optional): Default: False. If True, preload the entire dataset to the specified device. 
        device (torch.device, optional): Default: None. The device to preload the data to. Required if preload is True.
    """

    def __init__(self, root_dir, metadata_json_path,
                 transform=None, preload=False, device=None):
        ...
        # Default transform = dual ToTensor
        ...
        # Preload logic: apply dual transform
        if self.preload:
            print(f"Preloading dataset to {self.device}...")
            self.preloaded_samples = []

            for sample, (clean_path, other_path, score) in enumerate(self.samples, start=1):
                print(f"\rsample {sample}/{self.__len__()}", end='')
                img_clean = np.array(Image.open(clean_path))
                img_other = np.array(Image.open(other_path))

                if self.transform:
                    transformed = self.transform(image=img_clean, image_other=img_other)
                    img_clean = transformed["image"]
                    img_other = transformed["image_other"]
            ...
            for sample, (img_clean, img_other, score) in enumerate(self.preloaded_samples):
                print(f"\rsample {sample + 1}/{self.__len__()}", end='')
                self.preloaded_samples[sample] = (
                    img_clean.to(self.device, non_blocking=True),
                    img_other.to(self.device, non_blocking=True),
                    score.to(self.device, non_blocking=True)
                )
    ...
    def __getitem__(self, idx):
        if self.preload:
            ...
            return img_clean, img_other, tensor(score, dtype=float32)
